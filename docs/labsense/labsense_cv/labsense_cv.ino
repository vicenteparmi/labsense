...
void runExperiment()
{
    ...
    // Calculate the intervals
    interval = (1000000L / (0.05 * 1000 * 128L));
    ...
    const float max_voltage_run = (1 + 1) * 127;
    const float min_voltage_run = (-1 + 1) * 127;
    const float start_voltage_run = (0 + 1) * 127;

    static bool foward = true;

    while (n <= (3 - 1) * 2) // The number of cycles is multiplied by 2 because there are two scans per cycle, forward and reverse
    {
        // Buffer
        char buffer[16];
